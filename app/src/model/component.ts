...
export default class Component {
    ...
    static findNodeByOrdinal(ordinal: number, components: ComponentType[]): NodeType | null {
        for (let i = 0; i < components.length; i++) {
            for (let j = 0; j < components[i].vertices.length; j++) {
                if (components[i].vertices[j].ordinal === ordinal) {
                    return components[i].vertices[j];
                }
            }
        } 
        return null;
